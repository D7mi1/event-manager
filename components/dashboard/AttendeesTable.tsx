...
export function AttendeesTable() {
   ...
   // منطق الفلترة (كما هو في كودك الأصلي)
         ...
         <div className="flex flex-col md:flex-row gap-4 justify-between items-center bg-[#18181B] p-4 rounded-2xl border border-white/10">
            <div className="flex gap-2">
               {[{ id: 'all', label: 'الكل' }, { id: 'confirmed', label: 'مؤكد' }, { id: 'attended', label: 'حاضر' }].map(tab => (
                  <button key={tab.id} onClick={() => setFilterType(tab.id as any)} className={`px-4 py-2 rounded-lg text-xs font-bold border transition-all ${filterType === tab.id ? 'bg-white text-black' : 'bg-white/5 text-white/50 border-transparent'}`}>{tab.label}</button>
               ))}
            </div>
               ...
               <tbody className="divide-y divide-white/5">
                  {filteredAttendees.map(g => (
                     <tr key={g.id} className="hover:bg-white/5 group">
                        ...
                        {/* 🔥 منطقة الإجراءات المحدثة */}
                        <td className="p-4 text-center flex justify-center items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">

                           {/* 1. زر المشاركة الذكي (الجديد) */}
                           <ShareInvite
                              guestName={g.name}
                           ...
                           <button onClick={() => {
                              const tableName = g.seats && g.seats.length > 0 ? g.seats[0].table?.name : undefined;
                              window.open(generateWhatsAppLink(g.phone, g.name, eventDetails?.name || '', g.id, tableName), '_blank')
                           }} className="p-2 bg-[#25D366]/10 text-[#25D366] rounded-xl hover:bg-[#25D366] hover:text-white transition-all" title="واتساب مباشر">
                              <MessageCircle size={16} />
                           </button>

                           {/* 3. زر الحذف */}
                           <button onClick={() => { if (confirm('حذف؟')) deleteGuest(g.id) }} className="p-2 bg-red-500/10 text-red-500 rounded-xl hover:bg-red-500 hover:text-white transition-all">
                              <Trash2 size={16} />
                           </button>
                        </td>
                     </tr>
